...
namespace FDuke.Analysis.Nullability
...
def nonNullAssumption (locs : List Loc) (ℓ : Fact locs) (e : FExpr) : List Loc :=
  match e with
...
def exprConsequent (locs : List Loc) (ℓ : Fact locs) (e : FExpr) : Consequent locs :=
  let nonNull := nonNullAssumption locs ℓ e |>.filterMap locs.finIdxOf?
  fun j => if nonNull.contains j then .nonnull else .top

def evalExpr (locs : List Loc) (ℓ : Fact locs) : FExpr → NVal
  | .Null => .top
  | .Int _ => .nonnull
  | .Var x =>
      match locs.finIdxOf? x with
...
def nodeTransfer (locs : List Loc) (g : CFG) (n : NodeID) :
  ...
  | some .Skip | some (.Call _ _) | some .Invoke | none => ρ
...
private lemma nodeTransfer_mono (n : NodeID) :
  ...
  | some k =>
    ...
    | Skip 
    | Invoke
    | Call f s => apply Domain.ord_distr; assumption
    | Assign x e =>
...
lemma evalExpr_sound {locs : List Loc} {ℓ : Fact locs} {σ : State} {expr : FExpr} {v : Val}
    (hcoh : coh_self ℓ σ)
...
lemma nonNullAssumption_sound {locs : List Loc} {ℓ : Fact locs} {σ : State} {expr : FExpr} {n : Int}
    {i : Fin locs.length}
    (hcoh : coh ℓ σ)
...
lemma exprConsequent_sound {locs : List Loc} {ℓ : Fact locs} {σ : State} {expr : FExpr} {n : Int}
    (hcoh : coh ℓ σ)
    (heval : EvalExpr σ expr (Val.Int n))
    (htruthy : n ≠ 0) :
    coh_self (fun i => (
...
@[simp] private lemma DFA_transferAlong (cfg : WFCFG)
    ...
    (DFA cfg locs).transferAlong cfg.kappa.analysis e ℓ =
...
def semantics (hnd : locs.Nodup) :
    DFASemantics (ls := kappaLangSem cfg) cfg.kappa.analysis (DFA cfg locs) :=
  { Coh := coh
    preserve_entry := by
      intro σ hinit
      cases hinit
    ...
    preserve_step := by
      ...
      cases hstep with
      | inr l => grind -- call case, useless
      | inl l =>
      rcases l with hassign|hassume|h|h
      · obtain ⟨x, e', v, hkind, heval, rfl⟩ := hassign
        ...
        · rename_i _ i heq
          ...
          · intros j v' h hupd h'
            ...
            split_ifs at h
            · subst j
              refine evalExpr_sound hcoh.left heval h ?_
              obtain ⟨rfl⟩ := List.finIdxOf?_eq_some_iff.mp heq |>.left
              simpa using hupd
            · rename_i hne
              have hx_neq : x ≠ locs.get j := by
                apply List.finIdxOf?_nodup <;> trivial
              rw [State.updated_neq] at hupd
              · apply hcoh.left <;> trivial
              · exact hx_neq
          · intros j k n v' hupdj habs hn hupdk h'
            obtain ⟨rfl⟩ := h'
            simp only at hupdj
            split_ifs at hupdj
            · simp only at hupdj
              ...
              by_cases h_ki : k = i
              · subst k
                rw [List.get_eq_getElem, hget, State.updated_eq] at hupdk
                injection hupdk
                contradiction
              · have h_x_neq : x ≠ locs.get k := by apply List.finIdxOf?_nodup <;> trivial
                rw [State.updated_neq] at hupdk <;> try trivial
                apply exprConsequent_sound hcoh heval hn k .Null <;> grind
            · rw [State.updated_neq] at habs <;> trivial
      · obtain ⟨c, m, hkind, heval, hm, rfl⟩ := hassume
        simp only [hkind, coh, beq_iff_eq]
        split_ands
        · apply exprConsequent_sound <;> trivial
        · apply hcoh.right
      all_goals
        obtain ⟨hkind, rfl⟩ := h
        simp only [hkind]; trivial
    preserve_stutter := by
...
def checkExpr {locs : List Loc} (ℓ : Fact locs) : FExpr -> Bool
  | .Null => true
  | .Int _ => true
  | .Var _ => true
...
end FDuke.Analysis.Nullability
