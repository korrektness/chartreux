...
Correctness proof for the κ-shaped CFG rewrites emitted by `kappaEdges`,
in the *denotational summary step* architecture:

* `G_sum` (`cfg.analysis`) — plain κ edges *plus* the `.summary` edge.
  The summary edge's `LStep` (`SummaryStep`) is *denotational*: it means
  "a κ-admitted chain of inline body runs on the caller's own κ-graph,
  through a gadget whose edges are present". It is **not** a CEK run.
* `G_κ` (`cfg.kappa.analysis`) — the same graph minus the summary edge — is
  what analyses run on.
* `reachable_of_simulates` (`Chartreux.Analysis.Generic`) transports reachability
  from `G_sum` to `G_κ`. Plain edges simulate themselves; the summary edge
  is discharged by `summaryStep_refines_kappa`, a *proved* unfold of the
  denotation (`chain_replay` + `call_identity_lstep`) — the summary step is
  self-certifying, so no checkedness hypotheses remain in the refinement
  direction or in `kappa_rewrite_correct`.
...
/-- The store-only language semantics on the κ-graph of a caller CFG: every
    edge is `.plain`, and a loud step is a `KappaStep` (an `IntraStep`, or the
    store-identity step of a `Call` node); bookkeeping is a stutter. This is
    the `G_κ` counterpart of `fdukeLangSem` on `G_sum`, and the semantics
    `kappa_rewrite_correct` speaks about. -/
...
/-- A chain of `k` inline body runs threading the store: `σ` reaches `σ'`
    through `k` successive `enL ⇝ exL` κ-graph runs. -/
inductive InlineChain (cfg : WFCFG) (enL exL : NodeID) :
    Nat → State → State → Prop where
  | zero {σ} : InlineChain cfg enL exL 0 σ σ
  | succ {k σ σ₁ σ'} :
      InlineBodyRun cfg enL exL σ σ₁ →
      InlineChain cfg enL exL k σ₁ σ' →
      InlineChain cfg enL exL (k + 1) σ σ'
...
/-- A counted chain of inline body runs.  The natural-number index counts
    bodies, independently of the ghost counter, which records the invocations
    executed inside those bodies. -/
inductive CntInlineChain (cfg : WFCFG) (enL exL : NodeID) :
    Nat → CntState → CntState → Prop where
  | zero {c} : CntInlineChain cfg enL exL 0 c c
  | succ {k c c₁ c'} :
      CntInlineBodyRun cfg enL exL c c₁ →
      CntInlineChain cfg enL exL k c₁ c' →
...
theorem CntInlineChain.forget_count {cfg : WFCFG} {enL exL : NodeID}
  ...
  | zero => exact .zero
  | succ hrun htail ih =>
    rcases hrun with ⟨cm, hrun, hstep⟩
    refine .succ ⟨cm.1, lsteps_forget_count cfg hrun, ?_⟩ ih
    exact hstep.1

/-- **The denotational summary step** — the `LStep` of a `.summary` edge
    `en → r`. It asserts, *self-certifyingly*, everything the κ refinement
    needs: `en` is a `Call` node, the κ-gadget edges of a site `(en, enL,
    exL, r)` are present in the caller, and the store transformation is a
    κ-admitted chain of inline body runs through that gadget.

    The site data (`κ`, `enL`, `exL`) is existential rather than read off the
    edge label: the refinement direction (`summaryStep_refines_kappa`) only
    needs *some* present gadget carrying the chain, and the projection
    direction (`cek_projection`) produces the real site from builder
    provenance. Because the denotation carries its own gadget edges, the
    refinement consumes no checkedness hypotheses at all. -/
def SummaryStep (cfg : WFCFG) (en r : NodeID) (σ σ' : State) : Prop :=
  ∃ (f : String) (ℓ : Nat) (κ : InvKind) (enL exL : NodeID),
    cfg.val.nodeKind en = some (.Call f ℓ) ∧
    (∀ e ∈ kappaEdges κ en enL exL r, e ∈ cfg.val.edges) ∧
    ∃ k, κAdmits κ k ∧ InlineChain cfg enL exL k σ σ'

/-- The FDuke `LStep` on `G_sum`: ordinary control-flow (`.plain`) edges take
    an `IntraStep` (an `invoke` node is store-identity there — see
    `IntraStep`); the `.summary` edge of a `call` node takes the denotational
    `SummaryStep`. Note the signature: no family, no `self` tag — the summary
    edge's meaning is intrinsic to the caller's graph. -/
def fdukeLStep (cfg : WFCFG) (e : Edge) (σ σ' : State) : Prop :=
  match e.kind with
  | .plain   => IntraStep cfg.val e.src σ σ'
  | .summary => SummaryStep cfg e.src e.dst σ σ'

/-- The `LangSem` instance for `G_sum` of a caller CFG. Ordinary nodes step à
    la `Chartreux/Duke`; a `call` node's `.summary` edge carries the denotational
    `SummaryStep`; genuine bookkeeping is `LStutter` (`σ = σ`), matching
    Duke's `dukeLangSem`. -/
...
theorem lsteps_inline_cnt (cfg : WFCFG) (Φ : FunEnv) (site : CallGadget)
  ...
  · intro e he
    rcases CFG.kappa_edges_mem.mp he with ⟨he, hplain⟩
    apply CFG.kappa_edges_mem.mpr
    refine ⟨hfragment.1 e ?_, ?_⟩
    · exact he
    · simpa [Edge.shift] using hplain
  · intro n k hn
...
theorem lsteps_from_exit_cnt (cfg : WFCFG) {en ex r : NodeID}
  ...
  have hmem : (⟨ex, r, .plain⟩ : Edge) ∈ cfg.kappa.analysis.edges :=
    CFG.kappa_edges_mem.mpr ⟨hedge, rfl⟩
...
theorem seq_skip_terminal_shape (Φ : FunEnv) (startLam : Nat) (body : Stmt) :
  ...
  · intro e he; have := he; simp_all +decide only [Stmt.wfcfgFrom, ne_eq] ;
    unfold WFCFG.kappa at he; simp_all +decide only [cfgFrom_seq_skip_shape] ;
    unfold CFG.kappa at he; simp_all +decide only [List.filter_append, decide_true,
      List.filter_cons_of_pos, List.filter_nil, List.mem_append, List.mem_filter, decide_eq_true_eq,
      List.mem_cons, List.not_mem_nil, or_false] ;
    rcases he with ( ⟨ he₁, he₂ ⟩ | rfl );
    · obtain ⟨h₁, h₂⟩ := ( Stmt.wfcfgFrom Φ startLam body ).prop
      exact Nat.ne_of_lt (h₂.1 e he₁);
    · have h_body_wf : ∀ (body : Stmt),
          let g := Stmt.cfgFrom Φ startLam body
          g.entry < g.nodes.length ∧
            g.exit < g.nodes.length ∧
            (∀ e ∈ g.edges, e.src < g.nodes.length ∧ e.dst < g.nodes.length) := by
        intro body; exact (by
        have := (Stmt.wfcfgFrom Φ startLam body).prop; simp_all +decide [ CFG.WellFormed ] ;
        exact ⟨ this.1, this.2.2.2.2.2, fun e he => ⟨ this.2.1 e he, this.2.2.1 e he ⟩ ⟩);
      exact Nat.ne_of_lt ( h_body_wf body |>.2.1 );
  · intro e he hdest; have := CFG.kappa_edges_mem.mp he; simp_all +decide [ Stmt.wfcfgFrom ] ;
    have := ( Stmt.wfcfgFrom Φ startLam body ).prop ; simp_all +decide [ CFG.WellFormed ] ;
    unfold Stmt.wfcfgFrom at *; simp_all +decide [ cfgFrom_seq_skip_shape ] ;
    grind
...
theorem seq_skip_prefix_to_body (Φ : FunEnv) (startLam : Nat) (body : Stmt)
  ...
  have htarget : g.val.exit ≠ gs.val.exit := by
    ...
    exact Nat.ne_of_lt (body.wfcfgFrom Φ startLam).prop.2.2.2.2.2
  have hedges : ∀ e ∈ gs.kappa.analysis.edges, e.dst ≠ gs.val.exit →
      e ∈ g.kappa.analysis.edges := by
    intro e he hdst
    rcases CFG.kappa_edges_mem.mp he with ⟨he, hk⟩
    apply CFG.kappa_edges_mem.mpr
    refine ⟨?_, hk⟩
...
theorem standalone_seq_skip_exit_decompose (Φ : FunEnv) (startLam : Nat)
  ...
  · apply (seq_skip_cntStep_iff Φ startLam body
      (body.wfcfgFrom Φ startLam).prop.2.2.2.2.2).mp
    exact hs
  ...
  | @succ k c c₁ c'' hlink hrest ih =>
    ...
    · have hb := hback (by omega)
      have hmem : (⟨exL, enL, .plain⟩ : Edge) ∈ cfg.kappa.analysis.edges :=
        CFG.kappa_edges_mem.mpr ⟨hb, rfl⟩
...
theorem CntInlineChain.append {cfg : WFCFG} {enL exL : NodeID}
    ...
    (h₂ : CntInlineChain cfg enL exL k₂ c₂ c₃) :
    CntInlineChain cfg enL exL (k₁ + k₂) c₁ c₃ := by
  induction h₁ with
  | zero => simpa using h₂
  | succ hlink htail ih =>
    grind [CntInlineChain]
...
/-- A summary edge in a resolved program component is backed by a recorded
call gadget in its certificate-bearing counterpart.  This is the bridge from
the CEK call rule's `succSummary` premise to the gadget consumed by the
balanced projection's summary-step case. -/
theorem Program.family_summary_site (p : Program) {ref : CFGRef} {cfg : WFCFG}
    (hcfg : p.family.cfgAt? ref = some cfg) {e : Edge}
    (hedge : e ∈ cfg.val.edges) (hsummary : e.kind = .summary) :
    ∃ component site,
      p.generatedFamily.cfgAt? ref = some component ∧ component.wfcfg = cfg ∧
      site ∈ component.generated.callGadgets ∧ e ∈ site.edges := by
  obtain ⟨component, hcomponent, hwfcfg⟩ :=
    (p.family_cfgAt?_iff_generatedFamily ref cfg).mp hcfg
  rw [← hwfcfg] at hedge
  have he : e ∈ component.generated.cfg.edges := hedge
  obtain ⟨site, hsite, hsiteEdge⟩ := component.summary_has_gadget he hsummary
  exact ⟨component, site, hcomponent, hwfcfg, hsite, hsiteEdge⟩
...
theorem cnt_call_chain_replay (cfg : WFCFG) {f : String} {ℓ : Nat}
  ...
  | succ k =>
      ...
      have hmemIn : (⟨en, enL, .plain⟩ : Edge) ∈ cfg.kappa.analysis.edges :=
        CFG.kappa_edges_mem.mpr ⟨hedgeIn, rfl⟩
...
/-- The store-identity hop of a `Call` node across one of its κ-gadget
    `.plain` edges (`en → enL` to enter the inline copy, or the `?`/ε bypass
    `en → r`): a single loud `KappaStep` (its `Or.inr` branch) on the κ-graph.
    This is the step `summaryStep_refines_kappa`'s construction starts (and,
    for `k = 0`, ends) with. -/
  ...
  | @succ k σ σ₁ σ'' hlink hrest ih =>
    ...
    · -- last link: the pending `exL` action fires across `exL → r`
      ...
      have hmem : (⟨exL, r, .plain⟩ : Edge) ∈ cfg.kappa.analysis.edges :=
        CFG.kappa_edges_mem.mpr ⟨hout, rfl⟩
      exact LSteps.trans _ hrun
...
-- # The κ refinement: summary steps refine to κ-graph runs

/-- **κ refinement — proved, `sorry`-free.** A denotational summary step is
    simulated by a κ-graph run from the call node to its return node. The
    denotation is self-certifying, so no checkedness hypotheses are consumed:
    it carries the site's gadget edges and the κ-admitted chain itself.

    *Proof.* For `k = 0` the chain is trivial (`σ' = σ`) and `κAdmits κ 0`
    provides the `?`/ε bypass `en → r` — one `Call`-identity hop. For
    `k ≥ 1`, enter the copy across `en → enL` (`call_identity_lstep` on
    `kappaEdges_en_enL`), then replay the chain (`chain_replay`): the
    back-edge `exL → enL` for every link but the last is provided by
    `κAdmits_two_back` whenever it is needed (`k ≥ 2` forces `+`/ε), and the
    last link leaves across `exL → r` (`kappaEdges_exL_r`). -/
theorem summaryStep_refines_kappa (cfg : WFCFG) {en r : NodeID}
    {σ σ' : State} (hsum : SummaryStep cfg en r σ σ') :
    LSteps (ls := kappaLangSem cfg) cfg.kappa.analysis en σ r σ' := by
  obtain ⟨f, ℓ, κ, enL, exL, hkind, hgadget, k, hadm, hchain⟩ := hsum
  rcases Nat.eq_zero_or_pos k with rfl | hpos
  · -- k = 0: the chain is trivial and the gadget provides the bypass
    cases hchain
    exact call_identity_lstep cfg hkind
      (hgadget _ (κAdmits_zero_bypass en enL exL r hadm)) σ
  · -- k ≥ 1: enter the copy, then replay the chain
    have henter := call_identity_lstep cfg hkind
      (hgadget _ (kappaEdges_en_enL κ en enL exL r)) σ
    have hreplay := chain_replay cfg
      (hgadget _ (kappaEdges_exL_r κ en enL exL r)) hchain
      (fun h2 => hgadget _ (κAdmits_two_back en enL exL r hadm h2)) hpos
    exact LSteps.trans _ henter hreplay

-- # The projection obligation: CEK runs project onto `G_sum`
...
theorem Program.family_fn_kind (p : Program) {f : String} {F : WFCFG}
  ...
  specialize hF;
  unfold CFGFamily.cfgAt? at hF; simp_all +decide ;
...
/-- Every summary successor of a located node lives in the same CFG. -/
...
/-- Unfold a family summary-successor of a located node into the caller CFG
    summary edge that produces it. -/
theorem succSummary_mem_iff {fam : CFGFamily} {self : CFGRef} {cfg : WFCFG}
    (hcfg : fam.cfgAt? self = some cfg) {n : NodeID} {nr : NodeRef} :
    nr ∈ fam.succSummary (self, n) ↔
      ∃ e ∈ cfg.val.edges, e.src = n ∧ e.kind = .summary ∧ nr = (self, e.dst) := by
  unfold CFGFamily.succSummary
  rw [hcfg]
  simp only [List.mem_map, List.mem_filter, decide_eq_true_eq]
  constructor
  · rintro ⟨e, ⟨he, hsrc, hk⟩, rfl⟩
    exact ⟨e, he, hsrc, hk, rfl⟩
  · rintro ⟨e, he, hsrc, hk, rfl⟩
    exact ⟨e, ⟨he, hsrc, hk⟩, rfl⟩
    ...
    | @step k' cc c₁ ce hsuffix hstep hrest =>
      ...
      | @«call» a nr f ℓ F L σ₀ ρ₀ K₀ hkind hF hL hs =>
        ...
        have hb : cq.n.2 < Lo.val.nodes.length := by
          rw [hcqn]
          obtain ⟨ed, hed, hesrc, hkd, hnr⟩ := (succSummary_mem_iff hcfg).mp hs
          rw [hnr]; exact Lo.prop.2.2.1 ed hed
...
theorem balanced_project_invoke (p : Program)
  ...
  have hplain : ed.kind = .plain := by
    rcases hk : ed.kind with _ | _
    · rfl
    · obtain ⟨f0, ℓ0, hcall⟩ := cfg.prop.2.2.2.2.1 ed hed hk
      rw [hesrc] at hcall; rw [hnk] at hcall; exact absurd hcall (by simp)
  ...
  -- Store-only `G_sum` run: the invoke node is store-identity.
  ...
  have hfdukeWhole : LSteps (ls := fdukeLangSem cfg) cfg.analysis n σ n' σ' := by
    ...
    exact Or.inr (Or.inr (Or.inr ⟨hnk, rfl⟩))
  exact ⟨_, qT + 1, (σ', qT + 1),
...
/-- The heart of the call case: a *single* completed function call, viewed as
    the callee body run that `ret-call` closes off, projects onto the caller's
    `.summary` edge `n → nr.2`.  Its ambient effect is the chain of ambient
    invocations performed while running the callee (`AmbientEvolve key ρ ρc`),
    its κ shadow is the counted inline chain admitted by the callee's counting
    verdict, and its `fduke` step is the denotational `SummaryStep`.
...
theorem call_step_bpgoal (p : Program) (hchecked : FamilyChecked p.family)
  ...
  obtain ⟨hcb, callbackCount, cCallee, hcbCount, hcntCallee,
    hcCallee, _⟩ := hcallee
  subst cCallee
  obtain ⟨e, hedge, hesrc, hesum, hnr⟩ := (succSummary_mem_iff hcfg).mp hsucc
  obtain ⟨component, site, hcomponent, hwfcfg, hsite, hesite⟩ :=
    Program.family_summary_site p hcfg hedge hesum
  have heq : e = ⟨site.en, site.ret, .summary⟩ :=
    kappaEdges_summary site.kappa site.en site.enL site.exL site.ret e hesite hesum
  have hen : site.en = n := by rw [← hesrc, heq]
  have hret : site.ret = nr.2 := by rw [hnr, heq]
  ...
  have hsummary : SummaryStep cfg n nr.2 σ σc := by
    refine ⟨site.f, site.lamID, site.kappa, site.enL, site.exL,
      hkindCfg, hgadget,
      callbackCount, hadmit, ?_⟩
    exact hinline.forget_count
  have hsumMem : (⟨n, nr.2, .summary⟩ : Edge) ∈ cfg.analysis.edges := by
    change (⟨n, nr.2, .summary⟩ : Edge) ∈ cfg.val.edges
    have heSrc : e.src = n := hesrc
    have heDst : e.dst = nr.2 := by grind
    have heKind : e.kind = .summary := hesum
    simpa [← heSrc, ← heDst, ← heKind] using hedge
  have hfduke : LSteps (ls := fdukeLangSem cfg) cfg.analysis
      n σ nr.2 σc :=
    LSteps.single cfg.analysis (e := ⟨⟨n, nr.2, .summary⟩, hsumMem⟩) hsummary
...
/-- The call step case of `balanced_project`: a complete function call
    (`call … ret-call`) collapses onto its site's `.summary` edge as a
    `SummaryStep`, whose inline chain length is the κ-admitted invocation
    count read from the callee's counting verdict.  It splits (via
...
theorem balanced_project_call (p : Program) (hchecked : FamilyChecked p.family)
  ...
  -- Project the call portion `n → nr.2` onto its `.summary` step.
    ...
    | @step k' c c₁ c'' hsuffix hstep hrest =>
      ...
      | @assign n₀ nr x e v σ₀ ρ₀ K₀ hkind heval hs =>
        ...
        have hplain : ed.kind = .plain := by
          rcases hk : ed.kind with _ | _
          · rfl
          · obtain ⟨f, ℓ, hcall⟩ := cfg.prop.2.2.2.2.1 ed hed hk
            rw [hsrc] at hcall; rw [hnk] at hcall; exact absurd hcall (by simp)
        subst hnr
        have hedge : (⟨n, ed.dst, .plain⟩ : Edge) ∈ cfg.val.edges := by
...
/-- **The projection theorem — proved.** In a checked family, every
    ground-truth CEK run located in one caller CFG — starting and ending with
    the empty continuation and no ambient closure, as whole-program runs do —
    is an `LSteps` run on that CFG's `G_sum` under the denotational
    `fdukeLangSem`: intra-procedural steps map edge by edge, and each complete
    `call … ret-call` round trip collapses onto its site's `.summary` edge as
    a `SummaryStep` whose inline chain has the κ-admitted length pinned by the
    callee's counting verdict (`FamilyChecked`).
...
/-- **κ-rewrite correctness — proved, `sorry`-free.** Any post-fixpoint `rd`
    of an analysis `A` on the κ-graph is correct for `G_sum`-reachable
    states: if `(n, σ)` is reachable in the semantic anchor `G_sum` under the
    denotational `fdukeLangSem`, then `S.Corr (rd n) σ`.

    No checkedness hypotheses appear: the denotational summary step is
    self-certifying, so the refinement direction is unconditional. The κ
    contract and builder provenance are consumed *upstream*, where
    `G_sum`-reachability is produced from ground-truth CEK runs
    (`cek_projection`).

    *Proof.* `reachable_of_simulates` transports `G_sum`-reachability to the
    κ-graph — plain edges simulate themselves as single `KappaStep`s, summary
    edges via the proved `summaryStep_refines_kappa` — and `reachable_corr`
    finishes on the κ-graph. -/
theorem kappa_rewrite_correct (cfg : WFCFG)
  ...
  intro e σ₀ σ₁ hstep
  have hstep' : fdukeLStep cfg e.val σ₀ σ₁ := hstep
  cases hk : e.val.kind with
  | plain =>
    -- a plain `G_sum` edge is a κ-graph edge; its `IntraStep` is a `KappaStep`
    simp only [fdukeLStep, hk] at hstep'
    have hmem : e.val ∈ cfg.kappa.analysis.edges :=
      CFG.kappa_edges_mem.mpr ⟨e.prop, hk⟩
    exact LSteps.single cfg.kappa.analysis (e := ⟨e.val, hmem⟩)
      (Or.inl hstep')
  | summary =>
    -- the summary edge refines through its self-certifying denotation
    simp only [fdukeLStep, hk] at hstep'
    exact summaryStep_refines_kappa cfg hstep'

/-
`#print axioms` record:
  * `kappa_rewrite_correct` and the whole refinement direction
    (`summaryStep_refines_kappa`, `chain_replay`, `call_identity_lstep`) —
