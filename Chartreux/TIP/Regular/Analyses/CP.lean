...
open Analysis
open Generic
open Chartreux.TIP (tipLStep tipLStutter tipLangSem forCFG WFCFG)
...
/-- A bundled `Chartreux.Analysis` for constant propagation, parameterized by
    the variable list, a `Nodup` proof, and the underlying TIP CFG. -/
def cpAnalysis (vars : List String) (hnd : vars.Nodup) (cfg : WFCFG) :
    Chartreux.Analysis (g := cfg.analysis) NodeID Edge State :=
  { dfa          := cpDFA vars cfg
...
/-- TIP-facing wrapper around `Chartreux.analyze`: run the bundled CP
    analysis directly on a TIP `CFG`. -/
def cpAnalyzeCFG (vars : List String) (hnd : vars.Nodup) (cfg : WFCFG) :=
  Chartreux.analyze (cpAnalysis vars hnd cfg)
