...
`BuildSpec b s b' en ex` says: starting from builder `b`, the CFG-builder
recursion on statement `s` yields a new builder `b'`, and the resulting
sub-CFG has entry node `en` and exit node `ex`. The predicate carries
exactly the structural facts (which nodes and which edges have been
emitted) that downstream simulation proofs (`step_decorate`,
`cek_refines_cfg`) need to read off without re-doing case analysis on the
builder's behaviour.

It is intentionally a side-predicate: nothing in the runtime CFG type
changes, so consumers like `Worklist` and `CP` are unaffected.
-/
...
def BuilderWF (b : CFGBuilder) : Prop :=
  b.nextID = b.cfg.nodes.length

namespace BuilderWF

@[simp] lemma empty : BuilderWF CFGBuilder.empty := by
  simp [BuilderWF, CFGBuilder.empty]

lemma addNode {b} (h : BuilderWF b) (k : NodeKind) :
    BuilderWF (b.addNode k).fst := by
  simpa [BuilderWF, CFGBuilder.addNode] using h

lemma addEdge {b} (h : BuilderWF b) (s d : NodeID) (ek : EdgeKind) :
    BuilderWF (b.addEdge s d ek) := by
  simpa [BuilderWF, CFGBuilder.addEdge] using h

end BuilderWF

/-! ## The inductive `BuildSpec` -/

/-- Structural witness that `b'` is the result of building `s` on top of
    `b`, with sub-CFG entry `en` and exit `ex`. -/
inductive BuildSpec : CFGBuilder → Stmt → CFGBuilder → NodeID → NodeID → Prop where
  ...
  /-- `Assign x e` emits an `Assign` node followed by a fresh `Skip`
      successor connected by a `Normal` edge. The trailing `Skip`
      mirrors the CEK post-writeback state `⟨inl Skip, E', K⟩` produced
      by `Step.AssignD`, so that `LocatedAt`/`KontMatches.assignK` can
      land at a `.Skip`-kinded successor (see `Flow/Eval/Located.lean`). -/
...
/-! ## Builder-level shape lemmas

These are tiny but useful: they describe how `addNode`/`addEdge` move the
underlying CFG. -/
...
/-! ## Well-formedness propagation -/

theorem preserves_WF {b s b' en ex} (hbs : BuildSpec b s b' en ex)
    (hwf : BuilderWF b) : BuilderWF b' := by
  induction hbs with
  | skip b => exact BuilderWF.addNode hwf _
  | assign b x e =>
    exact BuilderWF.addEdge (BuilderWF.addNode (BuilderWF.addNode hwf _) _) _ _ _
  | decl b x e =>
    exact BuilderWF.addEdge (BuilderWF.addNode (BuilderWF.addNode hwf _) _) _ _ _
  | seq _ _ ih₁ ih₂ =>
    have h₁ := ih₁ hwf
    have h₂ := ih₂ h₁
    exact BuilderWF.addEdge h₂ _ _ _
  | if_ _ _ ih_t ih_f =>
    case if_ _ _ c _ _ _ _ _ _ _ _ =>
    have h_cond := BuilderWF.addNode hwf (.Cond c)
    have h_t := ih_t h_cond
    have h_f := ih_f h_t
    have h_skip := BuilderWF.addNode h_f .Skip
    repeat (apply BuilderWF.addEdge); exact h_skip
  | while_ _ ih_b =>
    case while_ _ _ c _ _ _ _ =>
    have h_cond := BuilderWF.addNode hwf (.Cond c)
    have h_b := ih_b h_cond
    have h_skip := BuilderWF.addNode h_b .Skip
    repeat (apply BuilderWF.addEdge); exact h_skip

/-! ## Monotonicity: nodes prefix, edges sublist -/
...
theorem entry_lt {b s b' en ex} (hbs : BuildSpec b s b' en ex)
    (hwf : BuilderWF b) : en < b'.cfg.nodes.length := by
  induction hbs with
  | skip b => grind [addNode_nodes, BuilderWF]
  | assign b x e
  | decl b x e =>
    simp [BuilderWF] at hwf
  ...
  | @if_ b b₁ b₂ c t f en_t ex_t en_f ex_f _ _ ih_t _ =>
    ...
    simp [addEdge_nodes, addNode_nodes, BuilderWF] at hwf ⊢
...
theorem exit_lt {b s b' en ex} (hbs : BuildSpec b s b' en ex)
    (hwf : BuilderWF b) : ex < b'.cfg.nodes.length := by
  induction hbs with
  | skip b => grind [addNode_nodes, BuilderWF]
  | assign b x e
  | decl b x e =>
  ...
  | @if_ b b₁ b₂ c t f en_t ex_t en_f ex_f _ _ _ _ =>
    -- ex = b₂.nextID = b₂.cfg.nodes.length (BuilderWF b₂)
    simp only [BuilderWF, addEdge_nodes, addNode_nodes, List.length_append, List.length_cons,
      List.length_nil, Nat.zero_add] at hwf ⊢
    have h_cond : BuilderWF (b.addNode (.Cond c)).fst := BuilderWF.addNode hwf _
    have h_t : BuilderWF b₁ := preserves_WF (by assumption) h_cond
    have h_f : BuilderWF b₂ := preserves_WF (by assumption) h_t
    rw [BuilderWF] at h_f
    grind
  | @while_ b b₁ c body en_b ex_b hbs_b _ =>
    ...
    simp only [BuilderWF, addEdge_nodes, addNode_nodes, List.length_append, List.length_cons,
      List.length_nil, Nat.zero_add] at hwf ⊢
    have h_cond : BuilderWF (b.addNode (.Cond c)).fst := BuilderWF.addNode hwf _
    have h_b : BuilderWF b₁ := preserves_WF hbs_b h_cond
    rw [BuilderWF] at h_b
    grind
...
`BuildSpec.assign` / `BuildSpec.decl` emit *two* nodes: the
`Assign`/`Decl` node at id `b.nextID` and a fresh `Skip` successor at
`b.nextID + 1`, joined by a `Normal` edge. The lemmas below extract the
relevant `nodeKind`/`hasEdge` facts on the resulting builder. -/
...
A small helper packaging the two monotonicity facts produced by
`BuildSpec` (`nodes_prefix`, `edges_sublist`). Used by
`LocatedAt.of_buildSpec` to lift `LocatedAt`/`KontMatches` from a
sub-builder's CFG to the surrounding outer CFG. -/
...
A version of `CFGBuilder.buildGraph` whose result type also carries a
`BuildSpec` witness. The original tuple-shaped `buildGraph` in
`Flow/Lang/CFG.lean` is unchanged (modulo the `While` exit-bug fix), so
existing consumers (`CP`, `Worklist*`, `BigCFG`) continue to compile
without modification. New simulation proofs that need to read off
structural facts of the build instead use `buildGraphSpec`. -/
...
/-- The legacy tuple-shaped builder is the projection of `buildGraphSpec`
    that drops the witness. Provided for callers that don't need the
    structural witness. -/
